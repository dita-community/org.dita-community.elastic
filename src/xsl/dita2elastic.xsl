...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:local="urn:ns:localfunctions"
  xmlns:fn="http://www.w3.org/2005/xpath-functions"
  exclude-result-prefixes="xs local fn"
  version="3.0"
  expand-text="yes">
  ...
  <xsl:template match="/*" priority="10">
    ...
    <xsl:variable name="resultData" as="node()*">
      <fn:array>
        <xsl:apply-templates>
    ...
    <xsl:sequence select="xml-to-json($resultData)"/>
  </xsl:template>
  ...
  <xsl:template match="@class" mode="ditaClass">
    <fn:array key="ditaclass">
      <xsl:for-each select="tokenize(., ' ')[fn:position() gt 1]">
        <fn:string>{.}</fn:string>
      </xsl:for-each>
    </fn:array>
  </xsl:template>
  ...
  <xsl:function name="local:getNodeId" as="xs:string">
    ...
    <xsl:variable name="baseId" as="xs:string"
      select="($context/@id, generate-id($context))[1]"
    />
    <xsl:variable name="result" as="xs:string"
      select="
      if ($context is $topicParent)
      then $context/@id
      else if (exists($topicParent))
      then ($topicParent/@id || '^' || $baseId)
      else $baseId
      "
    />
